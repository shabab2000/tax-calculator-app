...
export default function Economy({route,navigation}) {
...
fetch('https://taxcalculator.ml/inc_e.php?uid='+uid)
    ...
    function abc(x) {
      ...
      const handlePress = async () => {
        try {
            setLoading(true)
                let uid = await AsyncStorage.getItem("uid");
            fetch('https://taxcalculator.ml/incomee.php', {
        method: 'POST',
        headers: {
...
/>
          <View style={{paddingTop:15}}/>
            <View style={{display: 'flex',flexDirection: 'row',}}>
                <View style={{width: '50%',backgroundColor:'#FFCC00',borderRightWidth:1,borderColor:'#aaa'}}>
                    <Text style={{fontSize:16,textAlign: 'center',paddingVertical:10}}>เงินได้</Text>
                </View>
                
                <View style={{width: '50%',backgroundColor:'#FFDA48'}}>
          ...
          <View style={{paddingTop:20}}/>
            <View style={{display: 'flex',flexDirection: 'row',}}>
                ...
                <View style={{width:'35%',paddingHorizontal:15}}>
                    ...
                    <Text style={{fontSize:16,textAlign: 'center',}}>{ total.totals ? abc(total.totals) : total.totals == 0 ? <ActivityIndicator size='large' color='#0000ff'  />:0 }</Text>
                    </View>
                </View>
            </View>
