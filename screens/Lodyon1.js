...
import { StyleSheet, Text, View,TextInput,TouchableOpacity,ScrollView, ToastAndroid, Alert, ActivityIndicator} from 'react-native'
import AsyncStorage from "@react-native-async-storage/async-storage";
import { Picker } from '@react-native-community/picker';
import { ProgressDialog } from 'react-native-simple-dialogs';

export default function Lodyon1({route,navigation}) {
    ...
    const childs = child <=0 ? 0 : child ==1 ? 15000 :  child1*30000+15000;
    
    const sumAwn = awn*15000;
    const sumAwns = awns*15000;

    const disability1 = disability<=0 ? 0 : disability*30000;
    ...
    const loadType = async () => {
        try {
                let ty = await AsyncStorage.getItem("types");
                setTypes(ty);
        } catch (e) {
    ...
    const handlePress = async () => {
        try {
            ...
            fetch('https://taxcalculator.ml/lodyon1.php', {
        method: 'POST',
        headers: {
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
            <View style={{display: 'flex',flexDirection: 'row',}}>
                <View style={{paddingHorizontal:20,width:'65%'}}>
                    ...
                    <Text style={{fontSize:10}}>ร้อยละ 30 ของเงินได้</Text>
                </View>
                <View style={{width:'35%',paddingHorizontal:15}}>
                    <View style={{borderWidth:1,borderRadius: 10}}>
                        <Text style={{fontSize:16,textAlign: 'center',color:'blue'}}>{inc ? abc(inc*30/100): 0 }</Text>
                    </View>
                </View>
            </View>
            ...
            <View style={{display: 'flex',flexDirection: 'row',}}>
                <View style={{paddingHorizontal:20,width:'65%'}}>
                    ...
                    <Text style={{fontSize:10}}>30,000 บาท</Text>
                </View>
                <View style={{width:'35%',paddingHorizontal:15}}>
                    <View style={{borderWidth:1,borderRadius: 10}}>
                    <Text style={{fontSize:16,textAlign: 'center',}}>{abc(30000)}</Text>
                    </View>
                </View>
            </View>
        ...
        <Picker.Item label="คู่สมรสไม่มีเงินได้" value="คู่สมรสไม่มีเงินได้" />
            ...
            <View style={{display: 'flex',flexDirection: 'row',paddingTop:10}}>
                <View style={{paddingHorizontal:20,width:'65%'}}>
                    ...
                    <Text style={{fontSize:10}}>คนแรก 15,000 บาท </Text>
                    <Text style={{fontSize:10}}>คนที่ 2 เป็นต้นไป เกิดปี 2561 เป็นต้นไป คนละ 30,000 บาท</Text>
                </View>
...
:null}
            <View style={{display: 'flex',flexDirection: 'row',paddingRight:13}}>
                <View style={{paddingHorizontal:20,width:'69%'}}>
                    ...
                    <Text style={{fontSize:10}}>คนละ 15,000 บาท (อายุ 60 ปีขึ้นไป และมีเงินได้ไม่เกิน 15,000 บาทต่อครึ่งปี)</Text>
        ...
        <Picker.Item label="2 คน" value="2" />
            ...
            <View style={{display: 'flex',flexDirection: 'row',}}>
                <View style={{paddingHorizontal:20,width:'65%'}}>
                    ...
                    <Text style={{fontSize:10}}>ตามจำนวนจริงที่จ่ายจริง แต่ไม่เกิน 7,500 บาท</Text>
                </View>
                <View style={{width:'35%',paddingHorizontal:15}}>
                    <View style={{borderWidth:1,borderRadius: 10}}>
                    <TextInput
            ...
            <View style={{display: 'flex',flexDirection: 'row',}}>
                <View style={{paddingHorizontal:20,width:'70%'}}>
                    ...
                    <Text style={{fontSize:10}}>คนละ 30,000 บาท (มีเงินได้ไม่เกิน 15,000 บาทต่อครึ่งปี)</Text>
                </View>
                <View style={{width:'20%',paddingLeft:10,paddingRight:5}}>
                    <View style={{borderWidth:1,borderRadius: 10}}>
                    <TextInput
                        placeholder=""
                        style={{ fontSize: 16,textAlign: 'center',}}
